...
if __name__ == '__main__':
    # 0.png ~ 9.pngを実際に試す(predict関数に渡す)
    for i in range(0, 10):
        file_name = '{}.png'.format(i)
        img = Image.open(file_name)
        """読み込んだpngファイルを表示し、PGMを強制終了(import pdb;ではデバッグツールを起動できる)
        img.show()
        import sys;sys.exit()
        """
        # pngファイルを28*28の2次元のnumpy配列になおし、255で割る(0~1の値に収める(正規化))
        img = np.asarray(img) / 255
        """
        print(np.asarray(img))
        import sys;sys.exit()
        """
         # 784の要素をもつ1次元の配列に直す(これでpredict関数に渡せる)
